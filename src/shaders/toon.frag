...
void main() 
{
    ...
    for(int i=0; i < numLights; i++)
    {
        ...
        // Diffuse component
        float diffuselookup = (max(dot(n, l), 0.0)) * 0.5 + 0.5;
        vec3 diffuseComponent = texture(diffuseRamp, vec2(diffuselookup, 0)).rgb;
        ...
        // Specular component
        float specularlookup = pow(max(dot(e, r), 0.0), shininess);
        vec3 specularComponent = texture(specularRamp, vec2(specularlookup, 0)).rgb;
